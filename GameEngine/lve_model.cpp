...
namespace lve {
	...
	void LveModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		lveDevice.createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, statingBufferMemory);


		void* data;
		vkMapMemory(lveDevice.device(), statingBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t> (bufferSize));
		vkUnmapMemory(lveDevice.device(), statingBufferMemory);
		...
		vkFreeMemory(lveDevice.device(), statingBufferMemory, nullptr);
	}
